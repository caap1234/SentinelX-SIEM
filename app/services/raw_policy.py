...
class RawPolicy:
    ...
    - exim_mainlog: solo eventos de interés (rejects, bounces, auth fails, rate-limit, etc.)
    ...
    def decide(self, *, log_type: str, raw_line: str, pe: Optional[ParsedEvent]) -> RawPolicyDecision:
        ...
        if self._is_exim_like(log_type_norm=log_type_norm, pe=pe):
            ...
            # normalizamos direction (el parser manda "outbound"/"inbound" en MAIL-005)
            direction_raw = str(extra.get("direction") or "").strip().lower()
            direction = {
                "inbound": "in",
                "outbound": "out",
                "incoming": "in",
            ...
            # Señales directas del parser (MAIL-005 / rate-limit)
            kind = str(extra.get("kind") or "").strip().lower()
            event_type = str(extra.get("event_type") or "").strip().lower()
            action = str(extra.get("action") or "").strip().lower()
            ...
            interesting = is_rate_limit or any(
                kw in msg
                for kw in (
                    "rejected",
                    "reject",
                    ...
                    # útiles para discard / políticas / rate-limit
                    "discard",
                    "discarded",
                    "exceeded",
                    "too many",
            ...
            # Si es rate-limit, guardamos aunque direction no venga “perfecto”
            if is_rate_limit:
                return RawPolicyDecision(store=True, source_hint=source_hint)

            # Fallback: mail_flow fail también es evidencia
            if event_type == "mail_flow" and action in ("fail", "failed", "deny", "denied"):
                return RawPolicyDecision(store=True, source_hint=source_hint)
