...
class Event(Base):
    ...
    # Mantener como PK "lógica" para el ORM (aunque en DB el PK real será compuesto por partición)
    ...
    # Relación con carga (se puede mantener FK hacia log_uploads)
    ...
    # Relación con raw log (opcional) -> al particionar rawlogs, el FK duro deja de ser viable.
    # Se deja como referencia "soft".
    raw_id = Column(
        BigInteger,
        nullable=True,
        index=True,
    )
    # relationship "soft" (sin FK en DB). Si no lo usas, puedes quitarla.
    rawlog = relationship(
        "RawLog",
