...
# ============================================================
# MODE SELECTION & INPUTS PRELIMINARES
# ============================================================
echo
echo "== Modo de Instalación =="
echo "1) Instalación Rápida (Local Default - Zero Touch, contraseñas autogeneradas)"
echo "2) Instalación Personalizada (Localhost - Pide contraseñas y puertos)"
echo "3) Instalación Personalizada (Servidor - Dominio + Nginx público)"
prompt MODE_OPT "Elige (1/2/3)" "1" 0 0

MODE="FAST"
[[ "$MODE_OPT" == "2" ]] && MODE="LOCALHOST"
[[ "$MODE_OPT" == "3" ]] && MODE="SERVER"

info "Modo seleccionado: ${MODE}"

SKIP_GEOIP=0
SKIP_INTERACTIVE_CHECKS=0
DOMAIN=""

if [[ "${MODE}" == "FAST" ]]; then
  echo "=> Modo Rápido: Aplicando configuración por defecto y generando contraseñas..."
  ...
  UPLOADED_LOGS_DIR="/app/uploaded_logs"
  ...
  BACKEND_PORT="8000"
  FRONT_PORT="4321"
  FRONT_SRC="${FRONT_SRC_DEFAULT}"
  FRONT_DEPLOY_PATH="${ROOT_DIR}/public_html"
  ...
  SKIP_GEOIP=1
  SKIP_INTERACTIVE_CHECKS=1
else
  # Personalizado Local o Server
  # DB
  ...
  # Ports
  prompt BACKEND_PORT "BACKEND_PORT (host -> contenedor backend)" "8000" 0 0

  # Front build/deploy
  echo "== Frontend =="
  prompt FRONT_SRC "Ruta del frontend (proyecto Astro)" "${FRONT_SRC_DEFAULT}" 0 0
  prompt FRONT_DEPLOY_PATH "Ruta destino para copiar front/dist (Nginx root si SERVER o Local Nginx)" "${ROOT_DIR}/public_html" 0 0

  if [[ "${MODE}" == "LOCALHOST" ]]; then
    prompt FRONT_PORT "Puerto frontend local (Nginx container)" "4321" 0 0
  fi

  # Nginx config (solo Server)
  if [[ "${MODE}" == "SERVER" ]]; then
    echo "== Servidor / Nginx =="
    prompt DOMAIN "Dominio para Nginx (server_name)" "example.com" 0 0
    prompt FRONTEND_BASE_URL "FRONTEND_BASE_URL (para link de reset password, etc)" "https://${DOMAIN}/" 0 0
  else
    prompt FRONTEND_BASE_URL "FRONTEND_BASE_URL (para link de reset password, etc)" "http://localhost:${FRONT_PORT:-4321}/" 0 0
  fi
  ...
  if [[ -n "${SMTP_HOST}" ]]; then
    ...
    if confirm "¿Habilitar loopback SMTP vía host-gateway? (recomendado en cPanel local)" "y"; then
      ...
      prompt SMTP_LOOPBACK_DOMAIN "Dominio a mapear (ej: mail.tudominio.com)" "${SMTP_HOST}" 0 0
    fi
  fi
...
# ============================================================
# CHECKS (GeoIP + trust config + preflight mitigations)
# ============================================================
echo "== Configurando pre-requisitos =="
if [[ "${SKIP_GEOIP}" == "0" ]]; then
  check_geoip_files
else
  mkdir -p "${GEOIP_DIR}"
  touch "${COUNTRY_MMDB}" "${ASN_MMDB}" # Dummy files just in case
  warn "Omitiendo descargas GeoIP en modo FAST."
fi

ensure_trust_config_file

if [[ "${SKIP_INTERACTIVE_CHECKS}" == "0" ]]; then
  ...
  info "Modo FAST: omitiendo verificaciones interactivas de CSF/Nginx."
fi
...
if [[ "${MODE}" == "FAST" || "${MODE}" == "LOCALHOST" ]]; then
  # Generar config simple de nginx local
  cat > "${ROOT_DIR}/nginx-local.conf" <<EOF_NGINX
server {
    listen 80;
...
EOF_NGINX
  ...
  frontend_local:
    ...
    volumes:
      ...
      - ${FRONT_DEPLOY_PATH}:/usr/share/nginx/html:ro
    depends_on:
      - backend
    logging:
      driver: "json-file"
...
if [[ "${MODE}" == "FAST" || "${INITIAL_ADMIN_PASSWORD}" != "" ]]; then
  echo
  echo "============================================================"
  echo " CREDENCIALES (GUARDA ESTOS DATOS) "
